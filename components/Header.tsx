import React from 'react'
import { css } from '@emotion/css'
import tw from '@tailwindcssinjs/macro'
import Link from 'next/link'
import { useRouter } from 'next/router'
import { useConnection } from '../hooks'
export const Header = () => {
  const router = useRouter()
  const [connectionId] = useConnection()
  const isActive = (href: string) => router.pathname === href

  const SafeLink = Link
  return (
    <nav
      id="header"
      className={css(tw`bg-gray-800 fixed w-full z-10 top-0 shadow`)}
    >
      <div
        className={css(
          tw`w-full container mx-auto flex flex-wrap items-center mt-0 pt-3 pb-3 md:pb-0`
        )}
      >
        <div className={css(tw`w-1/2 pr-0`)}>
          <div className={css(tw`flex relative inline-block`)}>
            <div className={css(tw`relative text-sm text-gray-100`)}>
              <button
                id="userButton"
                className={css(tw`flex items-center focus:outline-none mr-3`)}
              >
                <svg
                  width="180"
                  height="40"
                  viewBox="0 0 180 40"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M1.58984 5.53284L24.6651 22.7827L13.2628 40L1.58984 5.53284Z"
                    fill="#1A1A1A"
                  />
                  <path
                    opacity="0.82"
                    d="M14.1091 0L24.6655 22.8154L0 34.4997L14.1091 0Z"
                    fill="#4D4D4D"
                  />
                  <path
                    d="M67.8039 12.2048C67.5333 12.3675 67.3641 12.5953 67.2287 12.8883L62.9317 22.4896C62.7626 22.8802 62.6272 23.2382 62.4919 23.5962C62.3566 23.9542 62.2889 24.182 62.1874 24.5726C62.1197 24.182 61.5784 22.8476 61.443 22.4896L56.8415 12.8557C56.74 12.6279 56.5708 12.4001 56.334 12.2048C56.0971 12.0095 55.7926 11.9119 55.3528 11.9119H51.2588L59.9543 29.0315L55.4881 38.3725H59.3791C59.8866 38.3725 60.2588 38.2748 60.4957 38.1121C60.7325 37.9168 60.9355 37.6564 61.1047 37.2984L72.8791 11.9444H68.6836C68.3791 11.9444 68.0746 12.0421 67.8039 12.2048Z"
                    fill="#FFF"
                  />
                  <path
                    d="M146.3 26.6558C146.029 26.3955 145.725 26.2002 145.352 26.07C144.98 25.9073 144.608 25.8422 144.202 25.8422C143.796 25.8422 143.39 25.9073 143.018 26.07C142.646 26.2327 142.341 26.428 142.07 26.6558C141.8 26.9162 141.597 27.2091 141.461 27.5671C141.326 27.9252 141.225 28.2832 141.225 28.6737C141.225 29.0643 141.292 29.4549 141.461 29.7803C141.597 30.1383 141.834 30.4313 142.07 30.6591C142.341 30.9195 142.646 31.1148 143.018 31.2449C143.39 31.3751 143.762 31.4728 144.202 31.4728C144.608 31.4728 144.98 31.4077 145.352 31.2449C145.725 31.1148 146.029 30.9195 146.3 30.6591C146.57 30.3987 146.773 30.1058 146.909 29.7803C147.078 29.4223 147.146 29.0643 147.146 28.6737C147.146 28.2832 147.078 27.9252 146.909 27.5671C146.74 27.2091 146.537 26.9162 146.3 26.6558Z"
                    fill="#FFF"
                  />
                  <path
                    d="M135.879 3.28723H131.041V31.2125H135.879V3.28723Z"
                    fill="#FFF"
                  />
                  <path
                    d="M179.729 5.14201C179.559 4.78399 179.323 4.45852 179.052 4.19815C178.781 3.93777 178.443 3.70994 178.071 3.57976C177.699 3.41702 177.293 3.35193 176.853 3.35193C176.447 3.35193 176.041 3.41702 175.669 3.57976C175.296 3.74249 174.992 3.93777 174.687 4.19815C174.417 4.45852 174.18 4.78399 174.044 5.14201C173.875 5.50002 173.808 5.89058 173.808 6.31369C173.808 6.70426 173.875 7.09482 174.044 7.45283C174.214 7.81085 174.417 8.10377 174.687 8.36415C174.958 8.62452 175.296 8.85235 175.669 8.98254C176.041 9.14527 176.447 9.21036 176.853 9.21036C177.293 9.21036 177.699 9.14527 178.071 8.98254C178.443 8.8198 178.781 8.62452 179.052 8.36415C179.323 8.10377 179.559 7.7783 179.729 7.45283C179.898 7.09482 179.999 6.7368 179.999 6.31369C179.999 5.92313 179.898 5.53257 179.729 5.14201Z"
                    fill="#FFF"
                  />
                  <path
                    d="M179.29 11.9445H174.451V31.2122H179.29V11.9445Z"
                    fill="#FFF"
                  />
                  <path
                    d="M169.95 19.6258C169.916 18.5192 169.713 16.4362 169.341 15.5249C168.968 14.6135 168.461 13.7999 167.784 13.1489C167.108 12.498 166.295 11.9772 165.348 11.6192C164.401 11.2612 162.675 11.0985 161.491 11.131C160.205 11.1636 158.344 11.4565 156.72 12.14C154.183 13.1815 153.168 14.1579 153.168 14.1579L155.198 17.38C155.57 17.1847 157.871 15.2319 161.559 14.8739C162.675 14.7763 164.198 15.1017 164.841 15.6876C165.957 16.7616 165.754 17.9659 165.822 18.6494C165.822 18.747 165.856 19.2027 165.889 19.8536C165.754 19.821 164.671 19.6583 164.299 19.5932C162.269 19.3979 160.882 19.3654 159.495 19.5281C153.269 20.3092 151.645 24.7682 152.965 27.8927C153.675 29.5526 155.164 30.7893 157.363 31.4077C160.104 32.1889 164.435 31.0823 166.126 28.7389C166.126 28.7714 166.363 29.9431 166.431 30.2686C166.532 30.6917 166.769 30.9521 167.04 31.0823C167.311 31.2125 167.683 31.2775 168.156 31.2775L170.525 31.245C170.525 31.245 170.085 28.5762 170.051 24.0196C170.051 23.0106 169.95 19.6258 169.95 19.6258ZM165.145 25.7446C164.908 25.9724 164.638 26.2002 164.367 26.4281C160.138 29.6827 156.957 28.153 156.653 26.1351C156.416 24.5403 158.04 22.8154 161.119 22.4573C162.032 22.3597 163.859 22.3272 165.923 22.7503C165.957 23.4663 165.957 24.2149 165.991 24.8984C165.788 25.1262 165.416 25.4842 165.145 25.7446Z"
                    fill="#FFF"
                  />
                  <path
                    d="M125.289 14.8414C124.646 13.9301 123.902 13.214 122.988 12.6607C122.109 12.1074 121.093 11.782 120.011 11.6192C118.657 11.4565 117.473 11.5541 116.458 11.8796C115.511 12.2051 114.327 12.7909 113.481 13.4419V7.42068V3.28723H108.812V31.2124H111.857C112.195 31.2124 112.432 31.1474 112.635 31.0172C112.838 30.887 112.973 30.6592 113.041 30.3337L113.244 29.0318L113.345 28.5111C113.548 28.7714 113.785 29.0318 114.022 29.2596C114.36 29.6177 114.766 29.9106 115.172 30.1709C115.578 30.4313 116.052 30.6592 116.56 30.8544C117.067 31.0497 117.609 31.1799 118.251 31.245C119.571 31.4077 120.755 31.3101 121.838 30.9521C122.921 30.5941 123.834 30.0082 124.612 29.1945C125.39 28.3809 126 27.4045 126.439 26.2328C126.879 25.0611 127.116 23.7592 127.15 22.3272C127.184 20.7649 127.048 19.3328 126.744 18.0961C126.372 16.8267 125.898 15.7201 125.289 14.8414ZM114.868 26.9163C114.259 26.6233 113.954 26.1351 113.515 25.5493V17.5753V16.5664C114.09 16.0131 114.8 15.5249 115.443 15.2645C125.289 11.8796 124.578 31.7332 114.868 26.9163Z"
                    fill="#FFF"
                  />
                  <path
                    d="M49.465 23.2058C49.1267 22.685 48.6868 22.2619 48.1455 21.8714C48.044 21.8063 47.9425 21.7412 47.841 21.6761C47.3673 21.3832 46.8598 21.1228 46.3184 20.9275C44.9312 20.4068 43.4086 20.0162 42.0214 19.6907C41.3109 19.528 40.668 19.3327 40.0928 19.1049C39.5176 18.8771 39.0439 18.6167 38.6718 18.2912C38.2996 17.9658 38.0966 17.5752 38.0627 17.1195C38.1304 16.5662 38.4688 16.078 39.0778 15.72C40.1267 15.004 42.5289 14.9389 43.8823 15.2644C44.4237 15.3945 44.9312 15.5247 45.371 15.6875C45.8109 15.8502 46.1831 15.9804 46.4876 16.078C47.0289 16.2733 47.5364 16.2733 47.8071 16.1431C47.9425 16.078 48.0778 15.9478 48.2131 15.7851L49.3635 13.7021C48.4838 13.0186 47.4011 12.4979 46.1492 12.0748C43.7808 11.2611 40.262 11.2285 37.8936 11.9771C36.8109 12.3026 35.8973 12.7583 35.1868 13.3115C34.4763 13.8648 33.9349 14.4832 33.5966 15.2318C33.2582 15.9478 33.089 16.729 33.1229 17.5101C33.1229 17.5426 33.1229 17.5752 33.1229 17.5752C33.1229 17.6403 33.1229 17.7054 33.1229 17.803C33.1567 18.6167 33.3259 19.3002 33.6642 19.8535C34.0026 20.4068 34.4424 20.895 34.9838 21.2855C35.2206 21.4483 35.4575 21.5785 35.6943 21.7086C36.0665 21.9365 36.4387 22.0992 36.8447 22.2619C37.5552 22.5223 38.2996 22.7827 39.0101 22.978C39.7545 23.1732 40.465 23.3685 41.2094 23.5313C41.9199 23.694 42.5966 23.8893 43.1718 24.1171C43.747 24.3449 44.2206 24.6053 44.5928 24.9308C44.965 25.2562 45.168 25.6794 45.2019 26.1676C45.168 26.3954 45.1003 26.6232 44.9988 26.851C44.8297 27.144 44.5928 27.4369 44.2545 27.6647C43.3748 28.218 42.3259 28.4133 41.1417 28.4458C40.262 28.4458 39.4838 28.3807 38.8748 28.218C37.8936 27.9576 37.0139 27.5671 36.2018 27.2091C35.8973 27.0789 35.6266 27.0138 35.3221 27.0138C35.0176 27.0138 34.7469 27.0789 34.5778 27.144C34.3748 27.2416 34.2056 27.3718 34.0702 27.5671L32.8184 29.7152C33.2582 30.0081 33.7319 30.301 34.2733 30.5614C34.8823 30.8543 35.559 31.1147 36.2695 31.31C36.98 31.5052 37.7244 31.7005 38.5026 31.7982C39.2808 31.9284 40.059 31.9609 40.871 31.9609C42.3936 31.9609 43.7131 31.7656 44.8635 31.4402C46.0139 31.1147 46.9613 30.6265 47.7395 30.0406C48.4838 29.4548 49.059 28.7713 49.465 27.9576C49.8372 27.1765 50.0064 26.2652 50.0064 25.3213C50.0064 25.2888 50.0064 25.2888 50.0064 25.2562C50.0064 25.1912 50.0064 25.1261 50.0064 25.061C49.9725 24.3449 49.8034 23.7265 49.465 23.2058Z"
                    fill="#FFF"
                  />
                  <path
                    d="M101.367 12.4981C100.251 11.8146 98.8973 11.4891 97.5778 11.4891C96.2244 11.4891 95.0402 11.8471 93.8898 12.5632C93.8898 12.5632 93.4161 12.8561 93.2131 13.0188C92.5364 13.5396 91.4537 14.7113 91.2168 15.1018C89.7958 11.0009 84.5176 10.4151 81.2356 13.0188C80.8296 13.3443 80.4574 13.6698 80.0852 14.0603L79.7469 12.6283C79.5439 12.075 79.104 11.7821 78.4611 11.7821H75.416V31.2451H80.4236V16.957C80.9649 16.3712 81.5401 15.9155 82.183 15.59C83.5025 14.8415 86.0063 14.9065 86.9537 15.9806C87.5288 16.5664 87.9349 17.5428 87.9349 18.9749V31.2125H91.9612H92.2995V18.9749V16.957C92.8409 16.3712 93.4161 15.9155 94.0589 15.59C95.3785 14.8415 97.8823 14.9065 98.8296 15.9806C99.4048 16.5664 99.8108 17.5428 99.8108 18.9749V31.2125H104.818V20.407C104.818 17.5103 104.48 16.0131 103.803 14.9065C103.093 13.8325 102.382 13.1165 101.367 12.4981Z"
                    fill="#FFF"
                  />
                </svg>
              </button>
            </div>
            <div className={css(tw`block lg:hidden pr-4`)}>
              <button
                id="nav-toggle"
                className={css(
                  tw`flex items-center px-3 py-2 border rounded text-gray-500 border-gray-600 hover:text-gray-100 hover:border-teal-500 appearance-none focus:outline-none`
                )}
              >
                <svg
                  className={css(tw`fill-current h-3 w-3`)}
                  viewBox="0 0 20 20"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <title>Menu</title>
                  <path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v-2z" />
                </svg>
              </button>
            </div>
          </div>
        </div>

        <div
          className={css(
            tw`w-full flex-grow lg:flex lg:items-center lg:w-auto hidden lg:block mt-2 lg:mt-0 bg-gray-800 z-20`
          )}
          id="nav-content"
        >
          <ul className={css(tw`lg:flex flex-1 items-center px-4 md:px-0`)}>
            <SafeLink href="/">
              <li className={css(tw`mr-6 my-2 md:my-0`)}>
                <a
                  href="#"
                  className={css(
                    isActive('/')
                      ? tw`text-blue-400 border-b-2`
                      : tw`text-gray-500`,
                    tw`block py-1 md:py-3 pl-1 align-middle no-underline hover:text-gray-100 border-blue-400 hover:border-blue-400`
                  )}
                >
                  <span className={css(tw`pb-1 md:pb-0 text-sm`)}>
                    Phone call
                  </span>
                </a>
              </li>
            </SafeLink>
            <SafeLink href="/phone">
              <li className={css(tw`mr-6 my-2 md:my-0`)}>
                <a
                  href="#"
                  className={css(
                    isActive('/phone')
                      ? tw`text-green-400 border-b-2 border-green-400`
                      : tw`text-gray-500`,
                    tw`block py-1 md:py-3 pl-1 align-middle  no-underline hover:text-gray-100  hover:border-green-400`
                  )}
                >
                  <span className={css(tw`pb-1 md:pb-0 text-sm`)}>
                    Phone(Client only)
                  </span>
                </a>
              </li>
            </SafeLink>
            <SafeLink href="/conversations">
              <li className={css(tw`mr-6 my-2 md:my-0`)}>
                <a
                  href="#"
                  className={css(
                    isActive('/conversations')
                      ? tw`text-pink-400 border-b-2 border-pink-400`
                      : tw`text-gray-500`,
                    tw`block py-1 md:py-3 pl-1 align-middle no-underline hover:text-gray-100   hover:border-pink-400 border-pink-400`
                  )}
                >
                  <span className={css(tw`pb-1 md:pb-0 text-sm`)}>
                    Conversation data
                  </span>
                </a>
              </li>
            </SafeLink>
            <SafeLink href="/text">
              <li className={css(tw`mr-6 my-2 md:my-0`)}>
                <a
                  href="#"
                  className={css(
                    isActive('/text')
                      ? tw`text-purple-400 border-b-2`
                      : tw`text-gray-500`,
                    tw`block py-1 md:py-3 pl-1 align-middle no-underline hover:text-gray-100   hover:border-purple-400 border-purple-400`
                  )}
                >
                  <span className={css(tw`pb-1 md:pb-0 text-sm`)}>Text</span>
                </a>
              </li>
            </SafeLink>
            <SafeLink href="/audio">
              <li className={css(tw`mr-6 my-2 md:my-0`)}>
                <a
                  href="#"
                  className={css(
                    isActive('/audio')
                      ? tw`text-pink-400 border-b-2`
                      : tw`text-gray-500`,
                    tw`block py-1 md:py-3 pl-1 align-middle no-underline hover:text-gray-100   hover:border-pink-400 border-pink-400`
                  )}
                >
                  <span className={css(tw`pb-1 md:pb-0 text-sm`)}>Audio</span>
                </a>
              </li>
            </SafeLink>
            <SafeLink href="/video">
              <li className={css(tw`mr-6 my-2 md:my-0`)}>
                <a
                  href="#"
                  className={css(
                    isActive('/video')
                      ? tw`text-purple-400 border-b-2`
                      : tw`text-gray-500`,
                    tw`block py-1 md:py-3 pl-1 align-middle no-underline hover:text-gray-100   hover:border-purple-400 border-purple-400`
                  )}
                >
                  <span className={css(tw`pb-1 md:pb-0 text-sm`)}>Video</span>
                </a>
              </li>
            </SafeLink>
          </ul>
        </div>
      </div>
    </nav>
  )
}
